from tkinter import Tk, Frame, Label, Entry, Button, StringVar, IntVar, Scrollbar, BOTH, EW, W, END, CENTER, NO
...
# object for database
data = Database(db='expensesdb.db')
...
total_spent = Button(
    ...
    text='Total Spent',
    font=f,
    command=lambda: messagebox.showinfo('Total Spent', f"Total Spent: {data.fetchRecord('select sum(item_price) from expense_record')[0][0]}")
)
